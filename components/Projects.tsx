...
const projectsData: projectsProps[] = [
  ...
  },
  {
    ...
    description:
      "Modern Portfolio Website UI Figma Design with Light / Dark Mode",
    price: "$15",
    buttonLabel: "Purchase Design",
    image: "/images/epiko-market.svg",
  },
  ...
  <div className="cursor-pointer bg-white rounded-3xl p-2 sm:p-3 shadow-lg w-full max-w-sm hover:bg-[#FFD54F] transition duration-300">
    ...
    <div className="flex flex-wrap justify-center gap-4 mt-6">
      {skills.map((skill) => (
        <div
          key={skill.name}
          className="flex items-center gap-2 px-3 py-1 bg-white rounded-full border border-mainColor hover:none"
        >
          <Image
            src={skill.image}
            alt={skill.name}
    ...
    <div className="flex items-center justify-center mt-2">
      <a
        href="#"
        target="_blank"
        className="text-white px-4 py-2 rounded-full text-lg font-bold"
      >
        <Image
